...
def start_data_logging():
    """Start logging IMU and GPS data to CSV file"""
    global data_logging_active, data_logging_thread, current_log_file, csv_writer, log_file_handle
    ...
    try:
        ...
        filename = f'imu_gps_log_{timestamp}.csv'
        filepath = os.path.join(DATA_LOG_DIR, filename)
        current_log_file = filepath
        
        # Open file and create CSV writer
        log_file_handle = open(filepath, 'w', newline='')
        ...
        data_logging_thread = threading.Thread(target=data_logging_loop, daemon=True)
...
def stop_data_logging():
    """Stop logging IMU and GPS data"""
    global data_logging_active, csv_writer, log_file_handle, current_log_file
    ...
    try:
        ...
        if data_logging_thread and data_logging_thread.is_alive():
            data_logging_thread.join(timeout=3.0)
        ...
        csv_writer = None
        print(f"Data logging stopped: {current_log_file}")
        current_log_file = None
...
def data_logging_loop():
    """Main loop for logging data every 2 seconds"""
    global csv_writer, log_file_handle
    
    print("Data logging loop started")
    ...
    while data_logging_active:
        try:
            ...
            imu_accX = 0.0
            imu_accY = 0.0
            if current_data.get('imu'):
                imu_heading = current_data['imu'].get('heading', 0.0)
                imu_accX = current_data['imu'].get('accX', 0.0)
                imu_accY = current_data['imu'].get('accY', 0.0)
            
            # Get GPS data
            gps_lat = 0.0
            gps_lon = 0.0
            if current_data.get('gps'):
                gps_lat = current_data['gps'].get('lat', 0.0)
                gps_lon = current_data['gps'].get('lon', 0.0)
            ...
            if current_data.get('fused'):
                fused_heading = current_data['fused'].get('heading', 0.0)
                fused_lat = current_data['fused'].get('lat', 0.0)
                fused_lon = current_data['fused'].get('lon', 0.0)
            
            # Write row to CSV
            if csv_writer and log_file_handle:
                csv_writer.writerow([
                    timestamp,
                    datetime_str,
                    imu_heading,
                ...
                # Optional: print confirmation (can be removed for production)
                # print(f"Logged data at {datetime_str}")
            
        except Exception as e:
            print(f"Error in data logging loop: {e}")
        
        # Wait 2 seconds before next log
        time.sleep(2.0)
    
    print("Data logging loop ended")

# ------------------ END DATA LOGGING FUNCTIONS ------------------ #
...
def start_logging():
    """Start logging IMU and GPS data"""
    success = start_data_logging()
    return jsonify({
        'success': success,
        'active': data_logging_active,
        'file': current_log_file if success else None
    })
...
def get_logging_status():
    ...
    return jsonify({
        ...
        'data_available': current_data.get('imu') is not None and current_data.get('gps') is not None
    })

@app.route('/data-logs', methods=['GET'])
def list_data_logs():
    """List all data log files"""
    try:
        logs = []
        if os.path.exists(DATA_LOG_DIR):
            for filename in sorted(os.listdir(DATA_LOG_DIR), reverse=True):
...
def delete_log(filename):
    """Delete a specific data log file"""
    try:
        filepath = os.path.join(DATA_LOG_DIR, filename)
        ...
        # Don't delete currently active log file
        if data_logging_active and filepath == current_log_file:
            return jsonify({'success': False, 'error': 'Cannot delete active log file'})
        
        if os.path.exists(filepath) and filename.endswith('.csv'):
            os.remove(filepath)
            print(f"Data log deleted: {filename}")
