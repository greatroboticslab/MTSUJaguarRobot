...
def on_connect(client, userdata, flags, rc, properties=None):
    ...
    if rc == 0:
        print(f"✓ MQTT Client connected successfully to {MQTT_BROKER}")
...
def on_message(client, userdata, msg):
    ...
    try:
        # Decode the JSON payload from the Raspberry Pi
        payload = json.loads(msg.payload.decode('utf-8'))
        
        # Add a local timestamp for logging accuracy
        # This records *when* the Jaguar received the data
        payload['Local_Timestamp'] = datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
        
        # Store the latest data (for the web endpoint)
        latest_sensor_data = payload 
        
        # Add the data to the log queue
        log_queue.append(payload)
        
    except Exception as e:
        print(f"Error processing MQTT message: {e}")

def sensor_logger_thread():
    """Background thread to handle saving sensor data from the queue to a CSV file."""
    
    # Define the fields expected from the Raspberry Pi's payload
    fieldnames = [
        'Local_Timestamp', 'Heading', 'Roll', 'Pitch', 'accX', 'accY', 'accZ', 
        'gps:Lat', 'Lat', 'Lon', 'Alt', 'Sats', 'Fix'
    ]
    
    # Check if the log file exists to decide if we need to write the header
    file_exists = os.path.exists(SENSOR_LOG_FILE)
    
    try:
        with open(SENSOR_LOG_FILE, 'a', newline='') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames, extrasaction='ignore')

            # Write the header row only if the file is new
            if not file_exists:
                writer.writeheader()
                print(f"Created new sensor log file: {SENSOR_LOG_FILE}")

            while True:
                # Process all data currently in the queue
                while log_queue:
                    data_row = log_queue.popleft()
                    try:
        ...
        print(f"CRITICAL: Sensor logging thread failed: {e}")

def start_mqtt_client_and_logger():
    """Initializes and starts the MQTT client and the logging thread."""
    
    # 1. Start the Sensor Logger Thread
    logger_thread = threading.Thread(target=sensor_logger_thread, daemon=True)
    logger_thread.start()
    print("✓ Sensor Logger Thread started.")
